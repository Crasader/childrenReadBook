...
public:
	...
	void httpChargerInfo();//获取充值信息和充值活动
...
//***********************************************************       管理队列        ****************************************************
	...
	int maxTask = 200;//最大任务数  超过最大任务 不接请求 并且清除准备队列
	...
	vector<HttpData*> m_myDeleteList;//全部待释放的对象
	vector<int> m_myDeleteId;//需要删除的任务
	bool controlRun =true;//控制网络请求是否启动

	bool addDownloadList(vector<int>& readylist);//从准备队列加入网络队列，返回是否满负荷并发
	HttpData* getHttpData(int id);
	...
	void addToDeleteHttpId(int id);//删除任务
	void addToDeleteHttpData(HttpData*);//删除任务
};
