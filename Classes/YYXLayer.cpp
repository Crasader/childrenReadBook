...
void YYXLayer::controlTouchTime(float outTime, string keyOfSaveTime, std::function<void()> runable, std::function<void()> unTouchRun)
{
	...
	int nowtime = CrossPlatform::getMillsTime()% 100000000;
...
void YYXLayer::deleteFileValue(string key, string dirPath)
{
	if (dirPath == "")
		dirPath = FileUtils::getInstance()->getWritablePath() + "Data";
	string path = dirPath + "/" + key;
...
void YYXLayer::CopyDirectory(string sourceDir, string destDir)
{
	...
	CrossPlatform::TraversingFiles(sourceDir, [destDir](string filepath, string filename) {
