...
void CityTableView::loadData()
{
	...
	m_data.clear();
	m_typeList.clear();
	auto _data = BookCityCache::getInstance()->getInfos();
	auto _storeids = BookCityCache::getInstance()->getSequence();
...
Layout* CityTableView::createItem(Size  mapdata, int idx)
{
	...
	if (m_typeList[idx] == _double)
	{
		...
		CityButtonInfo* data0 = BookCityCache::getInstance()->getCityButtonInfo(id0);
		CityButtonInfo* data1 = BookCityCache::getInstance()->getCityButtonInfo(id1);
		if (data0)//上面的按钮
		{
			//书店封面
