...
void Index::InitVIPCard()
{
	...
	auto listner1 = EventListenerCustom::create("showVIPRenew", [=](EventCustom* e) {
		if (App::m_debug != 0) {
		...
		if (User::getInstance()->getVip())
		{
			...
			auto times = User::getInstance()->getVipTime();
			if (times <= 86400 * 30)
			{
				if (times >= 86400 * 21) {
...
Layer* Index::WaitLayer()
{
	...
	auto listener = EventListenerCustom::create(TAG_DELETEWAITLAYER, [=](Ref* sender) {
		wait->removeFromParent();
	});
